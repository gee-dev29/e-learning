...
module.exports.register = async function (req, res) {
  try {
    if (
      ...
      res.status(400).send("Bad request, field input required.");
    } else {
      //hash password
...
module.exports.login = async function (req, res) {
  try {
    ...
    if (user) {
      ...
      return { status: res.status(403), message: "Invalid.. " };
    }
...
module.exports.getAllUser = async function (req, res) {
  try {
    ...
    const filterUser = allUser.filter((users) => users.role === "user");
...
module.exports.upload = async function (req, res) {
  try {
    ...
    const ifUser = checkUser.filter((users) => users.role === "user");
    if (ifUser) {
      const imageUpload = new User({
        image: {
          data: req.file.filesname,
